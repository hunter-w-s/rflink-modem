...
def _import_crypto_module(name: str):
...
def _resolve_module_and_cfg(cfg: Config):
    mod = _import_crypto_module(cfg.module)
    if not hasattr(mod, "Config"):
        raise AttributeError(f"crypto module '{cfg.module}' missing Config")
    if not hasattr(mod, "tx") or not hasattr(mod, "rx"):
        raise AttributeError(f"crypto module '{cfg.module}' missing tx/rx")
